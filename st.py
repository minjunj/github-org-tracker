...
def github_json_parse(parse, repo_name, start_date, end_date):
    ...
    for con_num in range(len(parse)):
            ...
            # Convert UNIX timestamps to YY-MM-DD format
            for item in data:
                if item['c'] != 0:
...
def overall(org, start_date_input, end_date_input, headers):
    ...
    # Assuming chart_placeholder.bar_chart is a placeholder for actual chart generation
    # Replace with your actual chart generation logic
...
# Get repository list and add 'Overall' option
name_list = ['Overall'] + get_repo(org, headers)

# User selection for repository view
selection = st.selectbox('Select Repository view:', name_list)

# Input form for start and end dates
...
if submit_button:
    if start_date < end_date:
        ...
        # Conditional logic to call the correct function based on the selection
        if selection == 'Overall':
            overall(org, start_date_str, end_date_str, headers)
        else:
            # Call specific_repo with the selected repository name
